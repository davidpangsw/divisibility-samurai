...
class _PlayAreaState extends State<PlayArea> with TickerProviderStateMixin {
  ...
  void _tryGenerateBlock() {
    ...
    // Auto generation with new limit (3 blocks)
    if (_blocks.length < Config.maxAutoGeneratedBlocks && 
  ...
  void _manualGenerateBlock() {
    final gameViewModel = Provider.of<GameViewModel>(context, listen: false);
    
    // Only generate blocks if game is active
    if (!gameViewModel.isGameActive) return;
    
    // Manual generation with higher limit (5 blocks)
    if (_blocks.length < Config.maxManualGeneratedBlocks) {
      final gameLevel = GameLevel.getLevel(gameViewModel.gameState.level);
      final block = BlockFactory.createBlock(gameLevel.divisor, gameViewModel.gameState.level);
    ...
    return SizedBox(
      ...
      child: Stack(
        children: [
          ...
          // Manual generation button at top
          Positioned(
            top: 10,
            left: _playAreaRectangle.width / 2 - 30, // Center horizontally
            child: Consumer<GameViewModel>(
              builder: (context, gameViewModel, child) {
                final canGenerate = gameViewModel.isGameActive && 
                                   _blocks.length < Config.maxManualGeneratedBlocks;
                return ElevatedButton(
                  onPressed: canGenerate ? _manualGenerateBlock : null,
